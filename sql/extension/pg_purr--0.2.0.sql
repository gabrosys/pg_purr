...
-- ========================================================================
-- Quantum query planner — returns the chosen join order as a table of
-- (step, table_name, estimated_cost) rows. Advisory only: the caller
-- must reorder the query themselves (or use quantum_query_rewrite
-- below).
--
-- The supported-query contract (single SELECT, INNER JOINs over base
-- tables, connected predicate graph) is enforced upfront by the
-- validator. PG's parser is consulted via plpy.prepare for syntax /
-- catalogue checks. Per-caller access is enforced by SECURITY INVOKER.
-- Schema-qualify table references — `public` is not in the function's
-- search_path. The quantum step runs on the local AerSimulator by
-- default; an IBM Quantum backend can be enabled by setting
-- QISKIT_IBM_TOKEN in the postgres process environment.
-- ========================================================================
...
-- ========================================================================
-- Quantum query rewriter — runs the same pipeline and returns the
-- rewritten SELECT, ready for EXECUTE. The rewrite is a left-deep
-- INNER-JOIN tree in the chosen order with the original ON predicates
-- pushed down to the earliest JOIN where every referenced relation is
-- in scope (semantics-preserving for INNER joins).
--
...
--
-- Rejects (with an error) anything the validator does not accept:
-- outer joins, NATURAL/USING, CROSS, CTEs, subqueries in FROM,
-- disconnected predicate graphs. Trivial inputs (≤ 1 relation) are
-- returned unchanged.
-- ========================================================================
